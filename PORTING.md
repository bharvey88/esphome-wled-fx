...
14. **The audio pipeline runs entirely on one task.** WLED splits it: the FFT
...
15. **`autoResetPeak()` uses a fixed 50 ms.** Upstream takes
...
16. **`FFT_PREFER_EXACT_PEAKS` is not optional.** Upstream can be built with a
